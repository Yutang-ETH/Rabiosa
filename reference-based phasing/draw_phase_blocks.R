...
pdf('phase_block.pdf', width = 10, height = 5)
par(mar=c(5, 5, 5, 4))
...
for(i in 1:7){
  ...
  rect(xleft = 1, ybottom = (8-i)*4 - 0.5, xright = block_largest[1, 5], ytop = (8-i)*4, col = "red2", border = NA)
...
pdf('total_length_reads_assigned.pdf', width = 5, height = 8)
par(mar=c(6, 6, 4, 6))
plot(1:sum(tag_length), 1:sum(tag_length), axes = F, ann = F, type = "n", xlim = c(0, 20), ylim = c(0, 100))
...
text(x = c(2, 8, 14), y = c(0, 0, 0), srt = 70, adj = c(1, 1), xpd = T, offset = 1,
     labels = c("Tagged", "Untagged", "Unaligned"), cex = 1.5)

axis(side = 2, 
     at = seq(0, 100, 20), 
     labels = c(0, paste(seq(20, 100, 20), "", sep = "")), 
     las = 2, cex.axis = 1.5)

title(ylab = "Total length of reads assigned (Gb)", cex.lab = 1.5)

legend("topright", legend = c("Haplotype 1", "Haplotype 2", "Untagged", "Unaligned"),
       fill = c("indianred2", "lightblue3", "gray80", "gray10"),
       x.intersp = 0.5, y.intersp = 0.8, bty = "n",
       yjust = 0.5, xpd=T, inset=c(-0.2,0.4), cex = 1.5)
dev.off()
